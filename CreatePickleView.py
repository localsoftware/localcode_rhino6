"""Provides a scripting component.
    Inputs:
        ...
        site_num: site number for file name
        layer: layer name for file name
        target_path: directory to save pickled file
        write: boolean to write pickle
    Output:
        out: status text
        geometry:
        view_target:
...
if target_path and layer and write:
    if site_num:
        f = open(target_path+str(int(site_num))+layer, 'wb')
    else:
        f = open(target_path+layer, 'wb')
    vec = [centroid[i]-cam_pos[i] for i in range(3)]
    new_data = [geometry,vec,centroid]
    pickle.dump(new_data, f)
    f.close()
    fcheck = open(target_path+str(int(site_num))+layer)
    geometry = pickle.load(fcheck)[0]
